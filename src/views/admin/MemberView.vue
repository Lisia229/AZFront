...
const submit = async () => {
  ...
  try {
    ...
    members[idx].image = members[idx].image || `https://api.dicebear.com/9.x/initials/svg?seed=${account.value}&backgroundColor=ffabab,ffdaab,ddffab,abe4ff,d9abff`
    form.editing = false
...
;(async () => {
  try {
    ...
    members.forEach(member => {
      member.image = member.image || `https://api.dicebear.com/9.x/initials/svg?seed=${member.account}&backgroundColor=ffabab,ffdaab,ddffab,abe4ff,d9abff`
    })
  } catch (error) {
    Swal.fire({
      icon: 'error',
